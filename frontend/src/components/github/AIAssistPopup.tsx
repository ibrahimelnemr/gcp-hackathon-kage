...
export function AIAssistPopup({ isOpen, onClose, repoUrl, taskDescription }: AIAssistPopupProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="relative bg-gray-900 text-gray-100 rounded-lg shadow-lg p-6 w-full max-w-5xl">
        <button
          onClick={onClose}
          ...
          {!isCommitted ? (
            <>
              ...
              {!isLoading && jsonChanges.length > 0 && (
                <div className="h-96 overflow-y-auto border rounded-md p-4 bg-gray-800 text-gray-100">
                  {jsonChanges.map((file, fileIndex) => (
                    <div key={fileIndex} className="mb-6">
                      ...
                      <pre className="w-full bg-gray-900 text-gray-100 border border-gray-700 rounded-md p-4 font-mono text-sm overflow-auto">
                        {file.changes
                          .map(
                            (change) =>
                              `${change.action.toUpperCase()} (Line ${change.line_number}): ${change.content}`
                          )
                          .join('\n')}
                      </pre>
                    </div>
